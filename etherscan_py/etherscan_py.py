...
class Client:
    ...
    def get(self, module, action, extra_data=""):
        url_prefix = "api" if self.network == "mainnet" else "api-" + self.network
    ...
    def get_all_events(
        ...
        events = self.threaded_search(
            self.get_events, address, from_block, to_block, thread_count, **kwargs
        )
        return events
    ...
    def threaded_search(
        self, fn_name, address, from_block, to_block, thread_count=1, **kwargs
    ):
        if from_block == 0:
        ...
        for i in range(thread_count):
            s, e = block_ranges[i]
            t = Thread(
                target=self.threaded_search_lock,
                args=(fn_name, lst, address, from_block, to_block, lock, kwargs),
            )
            threads.append(t)
    ...
    def threaded_search_lock(
        self, fn_name, lst, address, from_block, to_block, lock, args
    ):
    ...
    def get_transactions(self, address, from_block, to_block, args, results_copy=[]):
        ...
        extra_data = (
            f"address={address}&startblock={from_block}&endblock={to_block}&sort=asc"
        )
        res = self.get("account", "txlist", extra_data)
        ...
        if results == []:
            ...
            try:
                ...
                raise Exception(
                    f"Got bad response from Etherscan when querying logs for address {address}"
                )
...
if __name__ == "__main__":
    ...
    client.get_all_transactions(
        from_address="0xc6Fd786bB7416ece05e6691eE4967ad062393842", status=2
    )
